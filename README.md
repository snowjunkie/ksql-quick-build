...
10. You will see some internal kafka and ksql objects too, but you should see the following:
  - Topics - *orders*, *companies*, *spare*
  - Streams - *S_ORDERS*
  - Tables - *T_ORDERS*
11. If that worked, you are all set.
  - If it didn't, try looking at the logs using **docker-compose logs**
  - You can also look specifically at the logs for a given image by using **docker-compose logs [image-name]**
...
1. A *JSON* file is dropped into a parser target folder.
2. The parser will read each line of the file into an event.
3. Each event is submitted in *JSON* format to the **orders** topic, the *_id* field is defined as the kafka message key.
  - (Using the *_id* field as a message key ensures all events from the same order are managed by the same kafka broker)
4. A KSQL STREAM (**S_ORDERS**) is created from the **orders** topic, extracting only the *_id*, *state* and *email* data.
5. A KSQL TABLE (**T_ORDERS**) is created from the **S_ORDERS** stream containing the *state* and grouping by the *_id*.
  - This **T_ORDERS** table allows us to query the latest state for each order using the order ID.
